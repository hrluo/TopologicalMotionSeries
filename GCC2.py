...
def GCC2(dataset, filenam, threshold, maxscale, toll, lp, lq, Nsteps, lambda_list):
    #Dionysus2 only.
    ...
    with PdfPages(pdfnam) as pdf:
        ...
        print('Maximal scale:', float(maxscale))
        ...
        if threshold<0:
            ...
            bars = [bar for bar in dgms[1] if bar.death-bar.birth > threshold and bar.death-bar.birth < float(maxscale) ]
            cocycles = [cp.cocycle(bar.data) for bar in bars]
        ...
        print('Optimizer maximal iteration steps=',int(Nsteps))
        ...
        for lambda_parameter in lambda_list:
            ...
            overall_thr = float(threshold) #For the combined coordinates, we choose the global threshold.
            for s in vr:
                if s.dimension() != 1:
                    continue
